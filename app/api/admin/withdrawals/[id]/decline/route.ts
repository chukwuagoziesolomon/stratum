...
export async function POST(
  ...
  const reason = body.reason ? String(body.reason).trim() : "No reason provided";
  ...
  await pool.query("UPDATE withdrawals SET status = 'declined', reason = $1, processed_at = $2 WHERE id = $3", [reason, now, withdrawalId]);
