...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await pool.query("UPDATE transactions SET status = 'approved', processed_at = $1 WHERE id = $2", [now, depositId]);
  ...
  try {
    await sendDepositDecisionEmail({
      ...
      status: "approved",
      walletCoin: deposit.wallet_coin,
      walletNetwork: deposit.wallet_network,
    });
  } catch (error) {
    console.error("Deposit approval email failed:", error);
  }
